...
// input: { heartRate, stSegment, qtInterval, prInterval, qrsDuration,
//          rrInterval, complexes, spo2, temperature }
function assessRisk(input) {
  const hr = heartRate(input.heartRate);
  const st = stSegment(input.stSegment);
  const qt = qtInterval(input.qtInterval);
  const pr = prInterval(input.prInterval);
  const qrs = qrsDuration(input.qrsDuration);
  const rr = rrInterval(input.rrInterval);
  const cx = complexes(input.complexes);
  const sp = spo2(input.spo2);
  const temp = temperature(input.temperature);
